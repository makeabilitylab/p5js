//
// DrawTogether server: a tiny Express + Socket.io relay for a shared drawing canvas.
// It serves the browser client from public/ and relays 'mouse' draw events between
// all connected clients (each client's strokes appear on everyone else's canvas).
//
...
function onNewConnection(socket){
    ...
    // A client drew at (x, y). Rebroadcast it to every *other* client so they can
    // render the same stroke. broadcast.emit excludes the sender (who already drew it).
