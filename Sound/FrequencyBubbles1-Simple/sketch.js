/**
 * A simple artistic rendering of sound frequency using bubbles
 * 
...
function setup() {
  ...
  describe("An artistic rendering of the microphone's audio frequencies as bubbles that grow and shrink with the energy at each frequency.");
  ...
  for (let i = 0; i < numFftBins; i++) {
    ...
    const randHue = random(0, 360);
    const hueToFreq = map(freqAtBin, 0, numFftBins, 0, 360);
    //const allWhiteColor = color(360, 0, 100, 0.3);
    const fillColor = color(hueToFreq, 100, 100, 0.4);
    const bubble = new Bubble(randX, randY, freqAtBin, fillColor);
